...
----
----
Correct. The slide code should **not** assume `total_events` already exists if your source `df` is still at the raw auth/member level.

You need to **derive the scorecard first** from:

* `AuthznKey` = event grain
* `MemberID` = member grain

## Use this revised prep block first

This builds the full `scorecard_df` that the slide functions can consume.

```python
import pandas as pd
import numpy as np

def build_hospitalist_scorecard(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    From raw auth/member-level data, build:
    1) dfe: one row per authorization event (AuthznKey)
    2) scorecard_df: facility x hospitalist group summary
    """
    ...
    # Basic cleaning
    # -----------------------------
    x["AuthznKey"] = x["AuthznKey"].astype(str).str.strip()
    x["MemberID"] = x["MemberID"].astype(str).str.strip()
    ...
    x["AttendingProviderName"] = (
        x["AttendingProviderName"]
        .fillna("Unknown Provider")
        .astype(str)
        .str.strip()
    )

    x["AttendingProviderHospital_VOP"] = (
        x["AttendingProviderHospital_VOP"]
        .fillna("Unknown Hospital")
        .astype(str)
        .str.strip()
    )

    x["AttendingProviderHospitalGroupName_VOP"] = (
        x["AttendingProviderHospitalGroupName_VOP"]
        .fillna("Unassigned / Unknown")
        .astype(str)
        .str.strip()
    ...
    # Keep only Acute / OBS for this analysis
    x = x[x["BedType"].isin(["Acute", "OBS"])].copy()

    # -----------------------------
    # Build event-level dataset
    # -----------------------------
    # If your raw file is truly one row per AuthznKey already, this is harmless.
    # If you have repeated rows per auth event, this collapses to one event.
    #
    # We keep the first non-null-looking row after sorting. If you later discover
    # conflicting attributes within the same AuthznKey, that is a separate QA issue.
    # -----------------------------
    sort_cols = [
        "AuthznKey",
        "MemberID",
    ...
    dfe = (
        x.sort_values(sort_cols)
         .drop_duplicates(subset=["AuthznKey"], keep="first")
         .copy()
    )

    # -----------------------------
    # Facility x Group scorecard
    # total_events is CALCULATED here from distinct AuthznKey
    # -----------------------------
    scorecard_df = (
        dfe.groupby(
            ["AdmittingFacilityNameClean", "AttendingProviderHospitalGroupName_VOP"],
            dropna=False
        )
        .agg(
            total_events=("AuthznKey", "nunique"),
            unique_members=("MemberID", "nunique"),
            acute_events=("BedType", lambda s: (s == "Acute").sum()),
            obs_events=("BedType", lambda s: (s == "OBS").sum())
        )
        .reset_index()
    )
    ...
    # Facility totals
    facility_totals = (
        scorecard_df.groupby("AdmittingFacilityNameClean", dropna=False)["total_events"]
        .sum()
        .rename("facility_total_events")
        .reset_index()
    )

    scorecard_df = scorecard_df.merge(
        facility_totals,
        on="AdmittingFacilityNameClean",
        how="left"
    )

    # Derived metrics
    scorecard_df["events_per_member"] = (
        scorecard_df["total_events"] / scorecard_df["unique_members"].replace(0, np.nan)
    )

    scorecard_df["facility_share_pct"] = (
        scorecard_df["total_events"] / scorecard_df["facility_total_events"].replace(0, np.nan)
    )

    scorecard_df["obs_rate_pct"] = (
        scorecard_df["obs_events"] / scorecard_df["total_events"].replace(0, np.nan)
    )

    scorecard_df["acute_rate_pct"] = (
        scorecard_df["acute_events"] / scorecard_df["total_events"].replace(0, np.nan)
    )

    scorecard_df["group_rank_in_facility"] = (
        scorecard_df.groupby("AdmittingFacilityNameClean")["total_events"]
    ...
    scorecard_df = scorecard_df.sort_values(
        ["AdmittingFacilityNameClean", "group_rank_in_facility", "AttendingProviderHospitalGroupName_VOP"]
    ).reset_index(drop=True)

    return dfe, scorecard_df
```
...
```python
dfe, scorecard_df = build_hospitalist_scorecard(df)
...
Your `scorecard_df` will now have:

* `AdmittingFacilityNameClean`
* `AttendingProviderHospitalGroupName_VOP`
* `total_events`  ← now calculated from distinct `AuthznKey`
* `unique_members`
* `acute_events`
* `obs_events`
...
That is exactly what the slide code expects.

---

# Why this is the right approach

Because your raw file is at **auth/member level**, not summary level.

So:

* **event count** should come from `nunique(AuthznKey)`
* **member count** should come from `nunique(MemberID)`
* **Acute / OBS counts** should come from event-level records after collapsing to one row per `AuthznKey`

Otherwise you risk double counting the same hospitalization event.

---

# Revised styling / setup block

This replaces the earlier block that assumed `scorecard_df` already existed.
...
# Build the derived scorecard from raw df
dfe, scorecard_df = build_hospitalist_scorecard(df)

facility_order = (
    scorecard_df.groupby("AdmittingFacilityNameClean")["facility_total_events"]
    .max()
...
group_order = (
    scorecard_df.groupby("AttendingProviderHospitalGroupName_VOP")["total_events"]
    .sum()
...
scorecard_df["AdmittingFacilityNameClean"] = pd.Categorical(
    scorecard_df["AdmittingFacilityNameClean"],
    categories=facility_order,
    ordered=True
)

scorecard_df["AttendingProviderHospitalGroupName_VOP"] = pd.Categorical(
    scorecard_df["AttendingProviderHospitalGroupName_VOP"],
...
# Quick QA check before plotting

Run this to make sure the derived numbers are sane:

```python
print("Raw rows:", len(df))
print("Unique auth events:", df["AuthznKey"].nunique())
print("Event-level rows after collapse:", len(dfe))
print("Total events in scorecard:", scorecard_df["total_events"].sum())
...
* `len(dfe)` to roughly equal `df["AuthznKey"].nunique()`
* `scorecard_df["total_events"].sum()` to roughly equal number of unique events represented in your filtered acute/obs universe

---

# Important caveat

If the **same `AuthznKey` appears with conflicting group or facility values**, then `drop_duplicates(subset=["AuthznKey"])` is a practical shortcut, but it is also a red flag.

If you suspect that, run this QA:

```python
qa = (
    df.groupby("AuthznKey")
      .agg(
          facility_n=("AdmittingFacilityNameClean", "nunique"),
          group_n=("AttendingProviderHospitalGroupName_VOP", "nunique"),
          bed_n=("BedType", "nunique")
      )
      .reset_index()
)

conflicts = qa[(qa["facility_n"] > 1) | (qa["group_n"] > 1) | (qa["bed_n"] > 1)]
print("Conflicting AuthznKeys:", len(conflicts))
conflicts.head()
```

If that count is large, your event attribution needs cleanup before charting. Data loves to be dramatic.

---

# Bottom line

You were right. `total_events` should be **derived**, not assumed.

The correct flow is:

1. start with raw `df`
2. collapse to one row per `AuthznKey`
3. aggregate to facility × hospitalist group
4. calculate:
...
Then all the slide code works cleanly.

If you want, I can now rewrite the **slide plotting functions** so they all start from raw `df` directly instead of requiring `scorecard_df` as input.
